...
public class LaunchConfigLauncher extends LaunchConfigurationDelegate {
   ...
   public void launch(final ILaunchConfiguration config, final String mode, final ILaunch launch, final @Nullable IProgressMonitor monitor)
      ...
      final var dartMainFileRelativePath = config.getAttribute(Constants.LAUNCH_ATTR_DART_MAIN_FILE, "");
      final IFile dartMainFile;
      if (Strings.isBlank(dartMainFileRelativePath)) {
         /*
         dartMainFile = BuildSystem.DART.findDefaultBuildFile(project);
         ...
         dartMainFile = project.getFile(dartMainFileRelativePath);
         if (!dartMainFile.exists()) {
            Dialogs.showError("Dart file does not exist", "The configured dart file '" + dartMainFileRelativePath + "' does not exist.");
            return;
         }
      }
         ...
         case ILaunchManager.RUN_MODE:
            ...
            final var job = Job.create(NLS.bind(Messages.Launch_RunningFile, dartMainFile.getProjectRelativePath()), jobMonitor -> {
               try {
                  ...
                  processHandle.setAttribute("project_name", project.getName());
