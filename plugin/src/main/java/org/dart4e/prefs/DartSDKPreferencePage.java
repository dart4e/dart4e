...
public class DartSDKPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
   ...
   public Control createContents(final Composite parent) {
      ...
      sdkTable.addCheckStateListener(event -> {
         if (event.getChecked()) {
            final var sdk = (DartSDK) event.getElement();
            defaultSDK.set(sdk);
         } else {
            defaultSDK.set(null);
         }
      ...
      sdkTable.setContentProvider(new IStructuredContentProvider() {
         ...
         public Object[] getElements(final @Nullable Object input) {
            ...
            final var items = (Collection<DartSDK>) input;
      ...
      colName.setLabelProvider(new DelegatingStyledCellLabelProvider(new StyledLabelProviderAdapter()) {
         ...
         public StyledString getStyledText(final @Nullable Object element) {
            ...
            final var sdk = (DartSDK) element;
            return isDefaultDartSDK(sdk) //
   ...
   private boolean isDefaultDartSDK(final DartSDK sdk) {
   ...
   private void onButton_Edit() {
      ...
      final var sdk = asNonNullUnsafe((DartSDK) sel.getFirstElement());
      final var dialog = new DartSDKEditDialog(getShell(), sdk);
      if (dialog.open() == Window.OK) {
         sdks.remove(sdk);
         sdks.add(new DartSDK( //
            asNonNullUnsafe(dialog.sdkName.get()), //
   ...
   private void onButton_Remove() {
      ...
      if (sel.isEmpty())
         return;
      final var sdk = (DartSDK) sel.getFirstElement();
      sdks.remove(sdk);
      sdkTable.refresh();
   }
   ...
   public boolean performOk() {
      DartWorkspacePreference.setDartSDKs(sdks);
      final var sdk = defaultSDK.get();
      if (sdk != null) {
         DartWorkspacePreference.setDefaultDartSDK(sdk.getName());
      }
